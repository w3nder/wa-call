...
function sendReceipt(node: BinaryNode) {
  ...
  if (
    (node.content as any)[0].content[8] &&
    (node.content as any)[0].content[8].tag == "video"
  ) {
    event.type = "video";
  }
  baileys_sock.ev.emit("call", event);
...
function format_attrs(attrs: any): AttrsFormat {
